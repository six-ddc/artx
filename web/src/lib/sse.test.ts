...
describe('invalidationKeysForEvent', () => {
  ...
  it('maps `doc` (content) to invalidate(["doc", doc]) only', () => {
    expect(invalidationKeysForEvent('doc', { doc: 'a7f3k2', kind: 'content' })).toEqual([
      ['doc', 'a7f3k2'],
    ]);
  });

  it('maps `doc` (remap) to invalidate(["doc", doc]) AND ["comments", doc]', () => {
    expect(
  ...
  it('maps `doc` (aid/remove) to invalidate(["doc", doc]) only', () => {
    expect(invalidationKeysForEvent('doc', { doc: 'x', kind: 'aid' })).toEqual([['doc', 'x']]);
    expect(invalidationKeysForEvent('doc', { doc: 'x', kind: 'remove' })).toEqual([['doc', 'x']]);
  });
