...
export function ToolPill<T extends string>({
  ...
  return (
    <div className="fixed bottom-5 left-1/2 z-10 flex -translate-x-1/2 items-center gap-0.5 rounded-full border border-line bg-sheet p-1 shadow-lg">
      {tools.map(({ tool, label, icon: Icon }) => (
        <button
          ...
          className={cn(
            ...
            active === tool ? 'bg-marker/20 text-ink' : 'text-ink-2 hover:bg-hover hover:text-ink',
          )}
        >
