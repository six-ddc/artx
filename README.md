...
Agents are good at producing plans and prototypes and bad at knowing which paragraph you disliked. `art` closes that gap without a SaaS, an account, or a database.
...
```
          AGENT                                          HUMAN
            │                                              │
   art new payment-gateway                                 │
   writes index.md                                         │
            │ ───────────────  url  ─────────────────────▶ │
            │                                     reads at 127.0.0.1:7777
            │                                     selects a sentence, comments
            │ ◀────────────  anchored thread  ───────────  │
   art comments --open --json                              │
   edits the document                                      │
   art reply / art addressed                               │
            │ ────────────  live, over SSE  ─────────────▶ │
            │                                     art resolve
```
...
> **TODO** — `brew install art` and prebuilt release binaries are not available yet.

### Use

```bash
art init ~/vaults/work        # git init + directory skeleton + AGENTS.md
cd ~/vaults/work
art serve --open              # http://127.0.0.1:7777
```

Then, from wherever the agent works:

```console
$ art new payment-gateway --type md --json
{
  "id": "a7f3k2",
...
The agent writes the file at `path` with its own editing tools — `art` deliberately offers no content I/O — and hands you the `url`. You comment in the browser. The agent picks the comments up with `art comments --open --json`.
...
|---|---|
| `art init [dir]` | Create a vault: directory skeleton, `git init`, `.gitattributes`, `AGENTS.md` |
| `art new <slug> --type md\|html` | Allocate a doc id, write the skeleton file, print `{id, path, url}` |
| `art path <slug\|id>` | Resolve an artifact's absolute path |
| `art list` | List artifacts with their open-comment counts |
| `art open [slug\|id]` | Open the index or one artifact in a browser |
| `art serve` | Serve the vault: reading, commenting, SSE, file watcher |
| `art comments [--open\|--all] [--doc <slug>]` | List threads with path, line, offsets, quote and surrounding context |
| `art reply <thread> <text>` | Append a reply to a thread |
| `art addressed <thread> [--commit <sha>]` | Mark a thread handled (agents do this; resolving belongs to humans) |
| `art resolve <thread>` / `art reopen <thread>` | Close / reopen a thread |
| `art compact [--doc <slug>]` | Archive resolved threads and collapse edit/remap chains |
| `art doctor [--fix]` | Check the vault and repair what can be repaired automatically |
| `art watch --dispatch "<cmd>"` | Run a command whenever a new open comment appears |
| `art vault add\|list\|use` | Manage the global vault registry |

`art serve` flags: `--port` (default 7777), `--host` (requires `--token`), `--token`, `--no-watch`, `--open`, `--raw` (skip reviewer-script injection for HTML artifacts that misbehave in the sandbox).

Thread ids accept any unique prefix, so `art resolve c7k` works.

---

## Agent integration

`art init` writes an `AGENTS.md` into the vault describing the protocol. Point your agent at it — Claude Code, Codex, and anything else that reads a repo convention file will pick it up. In substance:

- When producing a plan or demo, run `art new <slug> --type md|html --json`, write the file at the returned `path`, and include the returned `url` in your reply.
- At the start of a session, run `art comments --open --json` and work through each thread: edit the document, `art reply <thread> "<what changed>"`, then `art addressed <thread> --commit <sha>`.
...
`art comments --open --json` gives the agent everything it needs to locate the feedback without guessing:
...
**One writer at a time.** When `art serve` is running it is the only process writing to comment files; the CLI probes for it via a lockfile and routes writes to its HTTP API. Without a running serve, the CLI appends directly while holding an advisory `flock`. Across machines, git handles it.
...
`art serve` binds `127.0.0.1` by default and refuses to start on a non-local address without `--token`. See [SECURITY.md](SECURITY.md).
